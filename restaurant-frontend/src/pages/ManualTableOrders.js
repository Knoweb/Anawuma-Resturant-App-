...
const ManualTableOrders = () => {
    ...
    const printOrder = (order, tableNo = null) => {
        ...
        const content = `
            <html>
                ...
                <body>
                    <div class="header">
                        <h2 style="margin:0; font-size:14px; font-weight:bold; text-transform:uppercase;">${restaurantName}</h2>
    ...
    const handleWhatsAppBill = (order) => {
        ...
        const locationLabel = order.roomNo ? `Room: ${order.roomNo}` : (order.tableNo ? `Table: ${order.tableNo}` : '–');
